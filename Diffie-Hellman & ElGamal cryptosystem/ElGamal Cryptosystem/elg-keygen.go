...
func generateG(p,q *big.Int) (*big.Int){
	...
	r:= big.NewInt(0).Div(order,new(big.Int).Mul(q,big.NewInt(2)))
...
func writetosecretKey(p,g, a big.Int, filename string){
	start := "("
	end := ")"
	comma := ","
	p_string := p.String()
...
func writetopublicKey(p,g, g_a big.Int, filename string){
	start := "("
	end := ")"
	comma := ","
	p_string := p.String()
...
func main(){
	...
	publicKey_filename := os.Args[1]
	secretKeyFilename := os.Args[2]
	var g_a big.Int
	p,q := generateP()
	g := generateG(&p,&q)
	...
	writetopublicKey(p,*g,g_a,publicKey_filename)
	writetosecretKey(p,*g,*a,secretKeyFilename)
}
